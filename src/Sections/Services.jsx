  ...
  return (
    <section className=' p-6 text-emerald-950 border-b-2 max-w-7xl mx-auto' id='services'>
      <div className="container mx-auto">
        <h2 className='my-8 text-center text-4xl  text-[#8F1413] font-bold sm:font-extrabold tracking-tighter uppercase'>Services</h2>
          ...
          {SERVICES.map((tab) => (
            <button
              ...
              className={`px-4 py-2 font-semibold lg:tracking-tighter transition-colors text-lg lg:text-2xl ${
                activeTab.id === tab.id ? "border-b-2 border-[#8F1413] text-[#8F1413]" : "text-[#103e60]"
              }`}
            >
              {tab.title}
            </button>
        ...
        key={activeTab.id} className="mt-8 flex flex-col items-center lg:flex-row">
          <div className="p-4 lg:w-1/2">
            <h2 className='mb-4 text-3xl text-[#8F1413] font-semibold lg:text-4xl'>{activeTab.content}</h2> {/* Fixed typo 'conent' to 'content' */}
